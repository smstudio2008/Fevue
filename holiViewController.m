...
@interface holiViewController ()<MHYahooParallaxViewDatasource,MHYahooParallaxViewDelegate,UITableViewDelegate,UITableViewDataSource>
...
@implementation holiViewController
    ...
    [query findObjectsInBackgroundWithBlock:^(NSArray *objects, NSError *error)
     {
         if (!error)
         {
             PFFile *file=[[objects objectAtIndex:10] valueForKey:@"background"];
             
             PFFile *file1=[[objects objectAtIndex:10] valueForKey:@"afterMovie"];
...
- (UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if(indexPath.row == 0){
        ...
        if(!cell) {
            ...
            UIImageView *map =[[UIImageView alloc] initWithFrame:CGRectMake(0, 308, 320, 180)];
              map.image=[UIImage imageNamed:@"maphol.jpg"];
  ...
  imageView.image = [UIImage imageNamed:@"hol.content.png"];
    }
...
- (void)showInstagramFeed {
    ...
    instagram.instagramUserId = @"176096097"; //if you want request the feed of one user
    
    
    
    //multiple users id or multiple tags (not both)
    instagram.instagramMultipleTags = @[@"holifestival", @"holifest", @"holifestivalofcolours"];
    ...
    [self presentViewController: instagram.feed animated:YES completion:nil];
     [Flurry logEvent:@"Explore instragram-Holi of color"];
}

- (void)buttonPressed:(id)sender {
  [self dismissViewControllerAnimated:YES completion:nil];
    [Flurry logEvent:@"Back to main menu button- Holi"];
}
...
-(void)lineup {
    ...
    JBWebViewController *controller = [[JBWebViewController alloc] initWithUrl:[NSURL URLWithString:@"http://holifestival.com/de/de/index"]];
    [controller showFromController:self];
    [Flurry logEvent:@"lineup- Holi"];
}


-(void)ticket {
    JBWebViewController *controller = [[JBWebViewController alloc] initWithUrl:[NSURL URLWithString:@"http://www.holifestival.com/de/de/news"]];
    [controller showFromController:self];
    [Flurry logEvent:@"ticket- Holi"];
}



-(void)aftermovie {
    XCDYouTubeVideoPlayerViewController *videoPlayerViewController = [[XCDYouTubeVideoPlayerViewController alloc] initWithVideoIdentifier:@"HeZKgmI4E3o"];
    [self presentMoviePlayerViewControllerAnimated:videoPlayerViewController];
    [Flurry logEvent:@"aftermovie- Holi"];
}


-(void)facebook {
    JBWebViewController *controller = [[JBWebViewController alloc] initWithUrl:[NSURL URLWithString:@"https://www.facebook.com/HoliFestivalOfColoursDeutschland"]];
     [controller showFromController:self];
    [Flurry logEvent:@"facebook- Holi"];
}



-(void)twitter {
    JBWebViewController *controller = [[JBWebViewController alloc] initWithUrl:[NSURL URLWithString:@"https://twitter.com/Holi_Festival"]];
        [controller showFromController:self];
    [Flurry logEvent:@"twitter- Hideout"];
}
...
- (void)map
{
    holimapViewController *google=[[holimapViewController alloc] initWithNibName:@"GoogleMapViewConttlloerViewController" bundle:nil];
    
    google.codinate = CLLocationCoordinate2DMake(51.036194,6.971886) ;
    google.codinate1 = CLLocationCoordinate2DMake(51.3181072,12.4029049) ;
    google.codinate2 = CLLocationCoordinate2DMake(51.0768337,13.7725857) ;
    google.codinate3 = CLLocationCoordinate2DMake(51.504883,7.055948) ;
    google.codinate4 = CLLocationCoordinate2DMake(49.232931,6.960618) ;
    google.codinate5 = CLLocationCoordinate2DMake(52.511908,13.240335) ;
    google.codinate6 = CLLocationCoordinate2DMake(53.4712405,-2.2968104) ;
    google.codinate7 = CLLocationCoordinate2DMake(49.003745,8.433467) ;
    google.codinate8 = CLLocationCoordinate2DMake(51.537773,7.45368) ;
    google.codinate9 = CLLocationCoordinate2DMake(49.468319,8.523856) ;
    
    
        google.direction2 = CLLocationCoordinate2DMake(51.504883,7.055948) ;
