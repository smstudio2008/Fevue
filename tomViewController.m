...
@interface tomViewController ()<MHYahooParallaxViewDatasource,MHYahooParallaxViewDelegate,UITableViewDelegate,UITableViewDataSource>
    ...
    [query findObjectsInBackgroundWithBlock:^(NSArray *objects, NSError *error)
     {
         if (!error)
         {
             PFFile *file=[[objects objectAtIndex:11] valueForKey:@"background"];
             
             PFFile *file1=[[objects objectAtIndex:11] valueForKey:@"afterMovie"];
...
- (UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if(indexPath.row == 0){
        ...
        if(!cell) {
            ...
            map.image=[UIImage imageNamed:@"maptom.jpg"];
        ...
        UIImageView *imageView = (UIImageView*)[cell viewWithTag:CONTENT_IMAGE_VIEW_TAG];
         imageView.image = [UIImage imageNamed:@"tom-content.png"];
    }
...
- (void)showInstagramFeed {
    ...
    instagram.instagramMultipleUsersId = @[@"290768179", @"366581549",@"201872192"];
    
    instagram.instagramMultipleTags = @[@"tomorrowland2014",@"tomorrowland2015"];
    ...
    [self presentViewController: instagram.feed animated:YES completion:nil];
       [Flurry logEvent:@"Explore instragram-TOMORROWLAND"];
}
...
-(void)lineup {
    ...
    JBWebViewController *controller = [[JBWebViewController alloc] initWithUrl:[NSURL URLWithString:@"http://www.tomorrowland.com/en/line-up"]];
    
   [controller showFromController:self];
           [Flurry logEvent:@"lineup- Tomorrowland"];
}


-(void)ticket {
    JBWebViewController *controller = [[JBWebViewController alloc] initWithUrl:[NSURL URLWithString:@"http://www.tomorrowland.com/home"]];
    [controller showFromController:self];
           [Flurry logEvent:@"ticket-Tomorrowland"];
}



-(void)aftermovie {
    XCDYouTubeVideoPlayerViewController *videoPlayerViewController = [[XCDYouTubeVideoPlayerViewController alloc] initWithVideoIdentifier:@"cUhPA5qIxDQ"];
    [self presentMoviePlayerViewControllerAnimated:videoPlayerViewController];
           [Flurry logEvent:@"aftermovie-Tomorrowland"];
}


-(void)facebook {
    JBWebViewController *controller = [[JBWebViewController alloc] initWithUrl:[NSURL URLWithString:@"https://www.facebook.com/tomorrowland"]];
    [controller showFromController:self];
           [Flurry logEvent:@"facebook- Tomorrowland"];    }



-(void)twitter {
    JBWebViewController *controller = [[JBWebViewController alloc] initWithUrl:[NSURL URLWithString:@"https://twitter.com/tomorrowland"]];
  [controller showFromController:self];
...
- (void)map
{
    ...
    google.codinate = CLLocationCoordinate2DMake(51.089413,4.382408) ;
    google.codinate1 = CLLocationCoordinate2DMake(51.089413,4.382408) ;
