...
@interface vfestViewController ()<MHYahooParallaxViewDatasource,MHYahooParallaxViewDelegate,UITableViewDelegate,UITableViewDataSource>
...
@implementation vfestViewController
    ...
    [query findObjectsInBackgroundWithBlock:^(NSArray *objects, NSError *error)
     {
         if (!error)
         {
             PFFile *file=[[objects objectAtIndex:6] valueForKey:@"background"];
             
             PFFile *file1=[[objects objectAtIndex:6] valueForKey:@"afterMovie"];
    ...
    if(indexPath.row == 0){
        ...
        if(!cell) {
            ...
            UIImageView *map =[[UIImageView alloc] initWithFrame:CGRectMake(0, 308, 320, 180)];
              map.image=[UIImage imageNamed:@"mapvfest.jpg"];
       ...
       imageView.image = [UIImage imageNamed:@"vfest-content.png"];
    }
...
- (void)showInstagramFeed {
  ...
  instagram.instagramMultipleTags = @[@"vfestival",@"vfestival2014",@"vfest",@"vfestival2015"];
    ...
    [self presentViewController: instagram.feed animated:YES completion:nil];
       [Flurry logEvent:@"Explore instragram-V FESTIVAL"];}
...
- (void)buttonPressed:(id)sender {
  ...
  [self dismissViewControllerAnimated:YES completion:nil];
     ...
     [Flurry logEvent:@"Back to main menu button- Vfest"];
}
...
-(void)lineup {
    ...
    JBWebViewController *controller = [[JBWebViewController alloc] initWithUrl:[NSURL URLWithString:@"http://www.vfestival.com/lineup"]];
        [controller showFromController:self];
    [Flurry logEvent:@"lineup- Vfest"];

}


-(void)ticket {
    JBWebViewController *controller = [[JBWebViewController alloc] initWithUrl:[NSURL URLWithString:@"http://www.vfestival.com/at-v-festival"]];
    [controller showFromController:self];
        [Flurry logEvent:@"ticket- Vfest"];
}



-(void)aftermovie {
    XCDYouTubeVideoPlayerViewController *videoPlayerViewController = [[XCDYouTubeVideoPlayerViewController alloc] initWithVideoIdentifier:@"OuFAnTrZbKM"];
    [self presentMoviePlayerViewControllerAnimated:videoPlayerViewController];
       [Flurry logEvent:@"aftermovie- Vfest"];
}


-(void)facebook {
    JBWebViewController *controller = [[JBWebViewController alloc] initWithUrl:[NSURL URLWithString:@"https://www.facebook.com/vfestival"]];
 [controller showFromController:self];
        [Flurry logEvent:@"facebook - Vfest"];
}



-(void)twitter {
    JBWebViewController *controller = [[JBWebViewController alloc] initWithUrl:[NSURL URLWithString:@"https://twitter.com/vfestival"]];
 [controller showFromController:self];
       [Flurry logEvent:@"twitter - Vfest"];
...
- (void)map
{
   ...
   google.codinate = CLLocationCoordinate2DMake(51.71042,-0.4354277) ;
    google.codinate1= CLLocationCoordinate2DMake(51.71042,-0.4354277) ;
     google.codinate2= CLLocationCoordinate2DMake(52.69539,-2.289826) ;
