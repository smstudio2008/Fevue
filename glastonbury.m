...
@interface glastonbury ()<MHYahooParallaxViewDatasource,MHYahooParallaxViewDelegate,UITableViewDelegate,UITableViewDataSource>
...
- (void)viewDidLoad
{
    ...
    _imageHeaderHeight = self.view.frame.size.height * 0.67f;
    ...
    [query findObjectsInBackgroundWithBlock:^(NSArray *objects, NSError *error)
     {
         if (!error)
         {
             PFFile *file=[[objects objectAtIndex:5] valueForKey:@"background"];
             
             PFFile *file1=[[objects objectAtIndex:5] valueForKey:@"afterMovie"];
...
- (UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if(indexPath.row == 0){
        ...
        if(!cell) {
            ...
            map.image=[UIImage imageNamed:@"mapglas.jpg"];
        ...
        imageView.image = [UIImage imageNamed:@"glas.content.png"];
    }
...
- (void)showInstagramFeed {
    ...
    instagram.instagramMultipleUsersId = @[@"1058408835",@"809211957"];
    
    instagram.instagramMultipleTags = @[@"glastonbury2014",@"glastonburyfestival",@"glastonbury2014"];    //if you set this you don't need set isSearchByTag in true
    ...
    [self presentViewController: instagram.feed animated:YES completion:nil];
        [Flurry logEvent:@"Explore instragram-GLASTONBURY"];
}

- (void)buttonPressed:(id)sender {
  [self dismissViewControllerAnimated:YES completion:nil];

          [Flurry logEvent:@"Back to main menu button- glastonbury"];    
}
...
-(void)lineup {
    ...
    JBWebViewController *controller = [[JBWebViewController alloc] initWithUrl:[NSURL URLWithString:@"http://www.glastonburyfestivals.co.uk/line-up/"]];
...
-(void)ticket {
    JBWebViewController *controller = [[JBWebViewController alloc] initWithUrl:[NSURL URLWithString:@"http://www.glastonburyfestivals.co.uk/information/"]];
 [controller showFromController:self];
}
...
-(void)aftermovie {
    XCDYouTubeVideoPlayerViewController *videoPlayerViewController = [[XCDYouTubeVideoPlayerViewController alloc] initWithVideoIdentifier:@"ZPusbdXLPWs"];
    [self presentMoviePlayerViewControllerAnimated:videoPlayerViewController];
}


-(void)facebook {
    JBWebViewController *controller = [[JBWebViewController alloc] initWithUrl:[NSURL URLWithString:@"https://www.facebook.com/glastonburyofficial"]];
    [controller showFromController:self];
...
-(void)twitter {
    JBWebViewController *controller = [[JBWebViewController alloc] initWithUrl:[NSURL URLWithString:@"https://twitter.com/glastofest"]];
    [controller showFromController:self];
...
-(void)showInstagramFeed:(id)sender
{
    [UIView animateWithDuration:0.4 animations:^{
        
        [_mapViewContainer setFrame:CGRectMake(0, 0,self.view.frame.size.width, self.view.frame.size.height)];
        
        
        MKPointAnnotation *point = [[MKPointAnnotation alloc] init];
        point.coordinate = CLLocationCoordinate2DMake(23.0, 73.0);
        point.title = @"Ahemedabad";
        ...
        region.span.latitudeDelta = 10.2;
        region.span.longitudeDelta = 10.2;
...
- (void)map
{
    ...
    google.codinate =  CLLocationCoordinate2DMake(51.158753,-2.583337);
     google.codinate1 =  CLLocationCoordinate2DMake(51.158753,-2.583337);
