...
@interface creamViewController ()<MHYahooParallaxViewDatasource,MHYahooParallaxViewDelegate,UITableViewDelegate,UITableViewDataSource>
    ...
    [query findObjectsInBackgroundWithBlock:^(NSArray *objects, NSError *error)
     {
         if (!error)
         {
             PFFile *file=[[objects objectAtIndex:1] valueForKey:@"background"];
             
             PFFile *file1=[[objects objectAtIndex:1] valueForKey:@"afterMovie"];
...
- (UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if(indexPath.row == 0){
        ...
        if(!cell) {
            ...
            map.image=[UIImage imageNamed:@"mapcream.jpg"];
        ...
        imageView.image = [UIImage imageNamed:@"cream-content.png"];
    }
...
- (void)showInstagramFeed {
    ...
    instagram.instagramMultipleUsersId = @[@"232012487",@"37495212"];
    instagram.instagramMultipleTags = @[@"creamfields",@"creamfields2014",@"creamfields14",@"creamfieldsuk",@"creamfields2015"];
    ...
    [self presentViewController: instagram.feed animated:YES completion:nil];
            [Flurry logEvent:@"Explore instragram-CREAMFIELD"];
}

- (void)buttonPressed:(id)sender {
  [self dismissViewControllerAnimated:YES completion:nil];
       [Flurry logEvent:@"Back to main menu button- creamfield"];    
}
...
-(void)lineup {
    ...
    JBWebViewController *controller = [[JBWebViewController alloc] initWithUrl:[NSURL URLWithString:@"http://www.creamfields.com/lineup"]];
    [controller showFromController:self];
        [Flurry logEvent:@"lineup- creamfield"];
}


-(void)ticket {
    JBWebViewController *controller = [[JBWebViewController alloc] initWithUrl:[NSURL URLWithString:@"http://www.creamfields.com/info"]];
    [controller showFromController:self];
     [Flurry logEvent:@"ticket- creamfield"];
}



-(void)aftermovie {
    XCDYouTubeVideoPlayerViewController *videoPlayerViewController = [[XCDYouTubeVideoPlayerViewController alloc] initWithVideoIdentifier:@"t6CFT5C6v7o"];
    [self presentMoviePlayerViewControllerAnimated:videoPlayerViewController];
    [Flurry logEvent:@"aftermovie- creamfield"];
}


-(void)facebook {
    JBWebViewController *controller = [[JBWebViewController alloc] initWithUrl:[NSURL URLWithString:@"https://www.facebook.com/OfficialCreamfields"]];
 [controller showFromController:self];
       [Flurry logEvent:@"facebook- creamfield"];
...
-(void)twitter {
    JBWebViewController *controller = [[JBWebViewController alloc] initWithUrl:[NSURL URLWithString:@"https://twitter.com/creamfields"]];
 [controller showFromController:self];
          [Flurry logEvent:@"twitter- creamfield"];
}
...
- (void)map
{
    ...
    google.codinate = CLLocationCoordinate2DMake(53.346222, -2.61796511) ;
    google.codinate1 = CLLocationCoordinate2DMake(53.346222, -2.61796511) ;
