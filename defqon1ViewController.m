...
#import "defqon1ViewController.h"
...
@interface defqon1ViewController ()<MHYahooParallaxViewDatasource,MHYahooParallaxViewDelegate,UITableViewDelegate,UITableViewDataSource>
...
@implementation defqon1ViewController
    ...
    [query findObjectsInBackgroundWithBlock:^(NSArray *objects, NSError *error)
     {
         if (!error)
         {
             PFFile *file=[[objects objectAtIndex:13] valueForKey:@"background"];
             PFFile *file1=[[objects objectAtIndex:13] valueForKey:@"afterMovie"];
    ...
    if(indexPath.row == 0){
        ...
        if(!cell) {
            ...
            imageView.tag = CONTENT_IMAGE_VIEW_TAG;
                cell.selectionStyle = UITableViewCellSelectionStyleNone;            [cell.contentView addSubview:imageView];
            ...
            UIImageView *map =[[UIImageView alloc] initWithFrame:CGRectMake(0, 308, 320, 180)];
               map.image=[UIImage imageNamed:@"mapdeq.jpg"];
        ...
        UIImageView *imageView = (UIImageView*)[cell viewWithTag:CONTENT_IMAGE_VIEW_TAG];
         imageView.image = [UIImage imageNamed:@"deq-content.png"];
    }
...
- (void)showInstagramFeed {
    ...
    instagram.instagramUserId =@"1319177544"; //if you want request the feed of one user
    
    
    instagram.instagramMultipleTags = @[@"defqon1",@"defqon1australia",
                                        @"defqon14"];
    ...
    [self presentViewController: instagram.feed animated:YES completion:nil];
     [Flurry logEvent:@"Explore instragram-DEFQON 1"];

}
- (void)buttonPressed:(id)sender {
  [self dismissViewControllerAnimated:YES completion:nil];
       [Flurry logEvent:@"Back to main menu button- Defqon"];
}
...
-(void)lineup {
    ...
    JBWebViewController *controller = [[JBWebViewController alloc] initWithUrl:[NSURL URLWithString:@"http://www.defqon1.nl/live-sets/"]];
    
    [controller showFromController:self];
       [Flurry logEvent:@"lineup- Defqon"];
}


-(void)ticket {
    JBWebViewController *controller = [[JBWebViewController alloc] initWithUrl:[NSURL URLWithString:@"http://www.defqon1.nl/info/"]];
    [controller showFromController:self];
           [Flurry logEvent:@"ticket - Defqon"];
}



-(void)aftermovie {
    XCDYouTubeVideoPlayerViewController *videoPlayerViewController = [[XCDYouTubeVideoPlayerViewController alloc] initWithVideoIdentifier:@"mwjMQTs2HfQ"];
    [self presentMoviePlayerViewControllerAnimated:videoPlayerViewController];
       [Flurry logEvent:@"aftermovie - Defqon"];
}


-(void)facebook {
    JBWebViewController *controller = [[JBWebViewController alloc] initWithUrl:[NSURL URLWithString:@"https://www.facebook.com/Defqon.1WeekendFestival"]];
    [controller showFromController:self];
       [Flurry logEvent:@"facebook - Defqon"];
...
-(void)twitter {
    JBWebViewController *controller = [[JBWebViewController alloc] initWithUrl:[NSURL URLWithString:@"https://twitter.com/Q_Dance"]];
     [controller showFromController:self];
           [Flurry logEvent:@"twitter  - Defqon"];
...
- (void)map
{
    ...
    google.codinate = CLLocationCoordinate2DMake(52.4396038,5.7682193) ;
    
     google.codinate1 = CLLocationCoordinate2DMake(52.4396038,5.7682193) ;
