...
@interface SgpViewController ()<MHYahooParallaxViewDatasource,MHYahooParallaxViewDelegate,UITableViewDelegate,UITableViewDataSource>
    ...
    [query findObjectsInBackgroundWithBlock:^(NSArray *objects, NSError *error)
     {
         if (!error)
         {
             PFFile *file=[[objects objectAtIndex:3] valueForKey:@"background"];
             
             PFFile *file1=[[objects objectAtIndex:3] valueForKey:@"afterMovie"];
    ...
    - (UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
        ...
        if(indexPath.row == 0){
            ...
            if(!cell) {
                ...
                map.image=[UIImage imageNamed:@"mapspg.jpg"];
            ...
            imageView.image = [UIImage imageNamed:@"sgp-content.png"];
        }
...
- (void)showInstagramFeed {
   ...
   instagram.instagramMultipleTags = @[@"secretgardenparty",@"secretgardenparty2014",];
    ...
    [self presentViewController: instagram.feed animated:YES completion:nil];
       [Flurry logEvent:@"Explore instragram-SGP"];}
...
- (void)buttonPressed:(id)sender {
  ...
  [self dismissViewControllerAnimated:YES completion:nil];
    ...
    [Flurry logEvent:@"Back to main menu button- SGP"];    
...
-(void)lineup {
    ...
    JBWebViewController *controller = [[JBWebViewController alloc] initWithUrl:[NSURL URLWithString:@"http://www.secretgardenparty.com/index.php/component/content/article?id=1"]];
    
    [controller showFromController:self];
        [Flurry logEvent:@"lineup- SGP"];
}


-(void)ticket {
    JBWebViewController *controller = [[JBWebViewController alloc] initWithUrl:[NSURL URLWithString:@"http://www.secretgardenparty.com/index.php/component/content/article?id=3"]];
    [controller showFromController:self];
    [Flurry logEvent:@"ticket- SGP"];
}



-(void)aftermovie {
    XCDYouTubeVideoPlayerViewController *videoPlayerViewController = [[XCDYouTubeVideoPlayerViewController alloc] initWithVideoIdentifier:@"RoLmtRkJqLI"];
    [self presentMoviePlayerViewControllerAnimated:videoPlayerViewController];
        [Flurry logEvent:@"aftermovie- SGP"];
...
-(void)facebook {
    JBWebViewController *controller = [[JBWebViewController alloc] initWithUrl:[NSURL URLWithString:@"https://www.facebook.com/secretgardenhq"]];
 [controller showFromController:self];
            [Flurry logEvent:@"Facebook- SGP"];
...
-(void)twitter {
    JBWebViewController *controller = [[JBWebViewController alloc] initWithUrl:[NSURL URLWithString:@"https://twitter.com/secretgardenhq"]];
 [controller showFromController:self];
        [Flurry logEvent:@"twitter- SGP"];}
...
- (void)map
{
    ...
    google.codinate = CLLocationCoordinate2DMake(52.3780718, -0.1732129) ;
     google.codinate1 = CLLocationCoordinate2DMake(52.3780718, -0.1732129) ;
