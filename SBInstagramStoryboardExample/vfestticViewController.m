...
@interface  vfestticViewController()

@end

@implementation  vfestticViewController
{
...
- (void)viewDidLoad
{
    ...
    if (SIMPLE_SAMPLE) {
        ...
        _glassScrollView = [[BTGlassScrollView alloc] initWithFrame:self.view.frame BackgroundImage:_imgView.image blurredImage:nil viewDistanceFromBottom:120 foregroundView:view];
        ...
        [_viewScroller setPagingEnabled:NO];
        [_viewScroller setDelegate:self];
        [_viewScroller setShowsHorizontalScrollIndicator:YES];
        [self.view addSubview:_viewScroller];
        
        _glassScrollView1 = [[BTGlassScrollView alloc] initWithFrame:self.view.frame BackgroundImage:[UIImage imageNamed:@"vfest-tic.jpg"] blurredImage:nil viewDistanceFromBottom:120 foregroundView:[self customView]];
...
- (UIView *)customView
{
    ...
    _bestivalLabel.text = @"V FESTIVAL";
    ...
    _price.text = @"VIAGAGO.CO.UK                               £227.27- £227.27 ";
    ...
    _price1.text = @"SEATWAVES                                     £89.00- £ 209.00 ";
    ...
    _price2.text = @"TICKETMASTER                                £89.00- £ 209.00 ";
    ...
    _price3.text = @"STUBHUB                                        £264.32- £ 271.40 ";
...
-(void)festivalPrice {
    JBWebViewController *controller = [[JBWebViewController alloc] initWithUrl:[NSURL URLWithString:@"http://www.vfestival.com/"]];
    [controller showFromController:self];
         [Flurry logEvent:@"ticket- vfestival-website-www.vfestival.com"];
}

-(void)searchPrice1 {
    JBWebViewController *controller = [[JBWebViewController alloc] initWithUrl:[NSURL URLWithString:@"http://www.viagogo.co.uk/Festival-Tickets/UK-Festivals/V-Festival-Tickets"]];
    [controller showFromController:self];
      [Flurry logEvent:@"ticket- vfestival-website-www.viagogo.co.uk"];
    
}

-(void)searchPrice2 {
    JBWebViewController *controller = [[JBWebViewController alloc] initWithUrl:[NSURL URLWithString:@"http://www.seatwave.com/eventfinder#genre=vfestival"]];
    [controller showFromController:self];
       [Flurry logEvent:@"ticket- vfestival-website-www.seatwave.com"];
    
}

-(void)searchPrice3 {
    JBWebViewController *controller = [[JBWebViewController alloc] initWithUrl:[NSURL URLWithString:@"http://www.ticketmaster.co.uk/V-Festival-tickets/artist/951997?tm_link=artist_artistvenue_module"]];
    [controller showFromController:self];
           [Flurry logEvent:@"ticket- vfestival-website-www.ticketmaster.co.uk"];
    
}

-(void)searchPrice4 {
    JBWebViewController *controller = [[JBWebViewController alloc] initWithUrl:[NSURL URLWithString:@"http://www.stubhub.co.uk/search/doSearch?searchStr=vfestival&pageNumber=1&resultsPerPage=50&searchMode=event&start=0&rows=50&geo_exp=1"]];
    [controller showFromController:self];
     [Flurry logEvent:@"ticket- vfestival-website-www.stubhub.co.uk"];
}
