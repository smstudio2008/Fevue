...
@interface fstvlViewController ()<MHYahooParallaxViewDatasource,MHYahooParallaxViewDelegate,UITableViewDelegate,UITableViewDataSource>
...
@implementation fstvlViewController
    ...
    [query findObjectsInBackgroundWithBlock:^(NSArray *objects, NSError *error)
     {
         if (!error)
         {
             PFFile *file=[[objects objectAtIndex:7] valueForKey:@"background"];
             
             PFFile *file1=[[objects objectAtIndex:7] valueForKey:@"afterMovie"];
...
- (UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if(indexPath.row == 0){
        ...
        if(!cell) {
            ...
            map.image=[UIImage imageNamed:@"mapfstvl.jpg"];
  ...
  imageView.image = [UIImage imageNamed:@"fstvl.content.png"];
    }
...
- (void)showInstagramFeed {
    ...
    instagram.instagramUserId = @"259430767"; //if you want request the feed of one user
    
    
    instagram.instagramMultipleTags = @[@"wearefstvl",@"wearefstvl2014",@"wearefstvl2015"];
    ...
    [self presentViewController: instagram.feed animated:YES completion:nil];
       [Flurry logEvent:@"Explore instragram-WE ARE FSTVL"];}
...
- (void)buttonPressed:(id)sender {
  ...
  [self dismissViewControllerAnimated:YES completion:nil];
    ...
    [Flurry logEvent:@"Back to main menu button- WEAREFSTVL"];
...
-(void)lineup {
    ...
    JBWebViewController *controller = [[JBWebViewController alloc] initWithUrl:[NSURL URLWithString:@"http://www.wearefstvl.com/lineup"]];
    
 [controller showFromController:self];
       [Flurry logEvent:@"lineup- WEAREFSTVL"];}


-(void)ticket {
    JBWebViewController *controller = [[JBWebViewController alloc] initWithUrl:[NSURL URLWithString:@"http://www.wearefstvl.com/info"]];
 [controller showFromController:self];
    
       [Flurry logEvent:@"ticket- WEAREFSTVL"];}



-(void)aftermovie {
    XCDYouTubeVideoPlayerViewController *videoPlayerViewController = [[XCDYouTubeVideoPlayerViewController alloc] initWithVideoIdentifier:@"HPlt_MIsfng"];
    [self presentMoviePlayerViewControllerAnimated:videoPlayerViewController];
           [Flurry logEvent:@"aftermovie- WEAREFSTVL"];}




-(void)facebook {
    JBWebViewController *controller = [[JBWebViewController alloc] initWithUrl:[NSURL URLWithString:@"https://www.facebook.com/wearefstvl"]];
 [controller showFromController:self];
    [Flurry logEvent:@"facebook- WEAREFSTVL"];}




-(void)twitter {
    JBWebViewController *controller = [[JBWebViewController alloc] initWithUrl:[NSURL URLWithString:@"https://twitter.com/wearefstvl"]];
 [controller showFromController:self];
    [Flurry logEvent:@"twitter- WEAREFSTVL"];}
...
- (void)map
{
    ...
    google.codinate = CLLocationCoordinate2DMake(51.5350867,0.2464752) ;
    google.codinate1 = CLLocationCoordinate2DMake(51.5350867,0.2464752) ;
