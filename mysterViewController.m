...
@interface mysterViewController ()<MHYahooParallaxViewDatasource,MHYahooParallaxViewDelegate,UITableViewDelegate,UITableViewDataSource>
...
@implementation mysterViewController
    ...
    [query findObjectsInBackgroundWithBlock:^(NSArray *objects, NSError *error)
     {
         if (!error)
         {
             PFFile *file=[[objects objectAtIndex:12] valueForKey:@"background"];
             
             PFFile *file1=[[objects objectAtIndex:12] valueForKey:@"afterMovie"];
...
- (UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if(indexPath.row == 0){
        ...
        if(!cell) {
            ...
            map.image=[UIImage imageNamed:@"mapmys.jpg"];
            ...
            _animatedLabel = [[ATLabel alloc] initWithFrame:CGRectMake(10, 270, 320, 100)];
    ...
    imageView.image = [UIImage imageNamed:@"mys-content.png"];
    }
...
- (void)showInstagramFeed {
    ...
    instagram.instagramUserId = @"347305751"; //if you want request the feed of one user
    
    instagram.instagramMultipleTags = @[@"mysteryland",@"mysterylandusa",@"mysterylandcl",@"mysteryland2014"
                                        ,@"mysterylandchile",@"mysterylandus",@"mysteryland14"];
    ...
    [self presentViewController: instagram.feed animated:YES completion:nil];
       [Flurry logEvent:@"Explore instragram-MYSTERLAND"];
}
...
- (void)buttonPressed:(id)sender {
  ...
  [self dismissViewControllerAnimated:YES completion:nil];
    ...
    [Flurry logEvent:@"Back to main menu button- Mysterland"];
}
...
-(void)lineup {
    ...
    JBWebViewController *controller = [[JBWebViewController alloc] initWithUrl:[NSURL URLWithString:@"http://www.mysteryland.nl/en/"]];
       [controller showFromController:self];
        [Flurry logEvent:@"lineup- Mysterland"];
}


-(void)ticket {
    JBWebViewController *controller = [[JBWebViewController alloc] initWithUrl:[NSURL URLWithString:@"http://www.mysteryland.nl/nl/info/"]];
        [controller showFromController:self];
       [Flurry logEvent:@"ticket- Mysterland"];}



-(void)aftermovie {
    XCDYouTubeVideoPlayerViewController *videoPlayerViewController = [[XCDYouTubeVideoPlayerViewController alloc] initWithVideoIdentifier:@"4VWfGgvHif0"];
    [self presentMoviePlayerViewControllerAnimated:videoPlayerViewController];
       [Flurry logEvent:@"aftermovie- Mysterland"];}


-(void)facebook {
    JBWebViewController *controller = [[JBWebViewController alloc] initWithUrl:[NSURL URLWithString:@"https://www.facebook.com/MysterylandHolland"]];
       [controller showFromController:self];
         [Flurry logEvent:@"facebook - Mysterland"];}



-(void)twitter {
    JBWebViewController *controller = [[JBWebViewController alloc] initWithUrl:[NSURL URLWithString:@"https://twitter.com/mysteryland"]];
      [controller showFromController:self];
             [Flurry logEvent:@"twiitter - Mysterland"];}
...
- (void)map
{
    ...
    google.codinate = CLLocationCoordinate2DMake(52.3302948,4.6683075) ;
     google.codinate1 = CLLocationCoordinate2DMake(52.3302948,4.6683075) ;
