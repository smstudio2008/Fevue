...
#import "tintheparkViewController.h"
...
@interface tintheparkViewController ()<MHYahooParallaxViewDatasource,MHYahooParallaxViewDelegate,UITableViewDelegate,UITableViewDataSource>
...
@implementation tintheparkViewController
    ...
    [query findObjectsInBackgroundWithBlock:^(NSArray *objects, NSError *error)
     {
         if (!error)
         {
             PFFile *file=[[objects objectAtIndex:4] valueForKey:@"background"];
             PFFile *file1=[[objects objectAtIndex:4] valueForKey:@"afterMovie"];
    ...
    if(indexPath.row == 0){
        ...
        if(!cell) {
            ...
            UIImageView *map =[[UIImageView alloc] initWithFrame:CGRectMake(0, 308, 320, 180)];
              map.image=[UIImage imageNamed:@"maptpark.jpg"];
        ...
        UIImageView *imageView = (UIImageView*)[cell viewWithTag:CONTENT_IMAGE_VIEW_TAG];
         imageView.image = [UIImage imageNamed:@"tinpark-content.png"];
    }
...
- (void)showInstagramFeed {
    ...
    instagram.instagramMultipleTags = @[@"tinthepark",@"tinthepark2014",@"tinthepark2015"];
   ...
   [Flurry logEvent:@"Explore instragram-T IN PARK"];}
...
- (void)buttonPressed:(id)sender {
  ...
  [self dismissViewControllerAnimated:YES completion:nil];
        [Flurry logEvent:@"Back to main menu button- TINTHEPARK"];
}
...
-(void)lineup {
    ...
    JBWebViewController *controller = [[JBWebViewController alloc] initWithUrl:[NSURL URLWithString:@"http://www.tinthepark.com/line-up.aspx"]];
    
    [controller showFromController:self];
     [Flurry logEvent:@"lineup - TINTHEPARK"];
}


-(void)ticket {
    JBWebViewController *controller = [[JBWebViewController alloc] initWithUrl:[NSURL URLWithString:@"http://www.tinthepark.com/essentials.aspx"]];
    [controller showFromController:self];
      [Flurry logEvent:@"ticket - TINTHEPARK"];
}



-(void)aftermovie {
    XCDYouTubeVideoPlayerViewController *videoPlayerViewController = [[XCDYouTubeVideoPlayerViewController alloc] initWithVideoIdentifier:@"wt6h56lXHO8"];
    [self presentMoviePlayerViewControllerAnimated:videoPlayerViewController];
     [Flurry logEvent:@"aftermovie - TINTHEPARK"];
}


-(void)facebook {
    JBWebViewController *controller = [[JBWebViewController alloc] initWithUrl:[NSURL URLWithString:@"https://www.facebook.com/tinthepark"]];
 [controller showFromController:self];
         [Flurry logEvent:@"facebook - TINTHEPARK"];
}



-(void)twitter {
    JBWebViewController *controller = [[JBWebViewController alloc] initWithUrl:[NSURL URLWithString:@"https://twitter.com/tinthepark"]];
 [controller showFromController:self];
        [Flurry logEvent:@"twitter - TINTHEPARK"];
...
- (void)map
{
    ...
    google.codinate = CLLocationCoordinate2DMake(56.3195027,-3.7494232) ;
    google.codinate1 = CLLocationCoordinate2DMake(56.3195027,-3.7494232) ;
