...
#import "hideoutViewController.h"
...
@interface hideoutViewController ()<MHYahooParallaxViewDatasource,MHYahooParallaxViewDelegate,UITableViewDelegate,UITableViewDataSource>
...
@implementation hideoutViewController
    ...
    [query findObjectsInBackgroundWithBlock:^(NSArray *objects, NSError *error)
     {
         if (!error)
         {
             PFFile *file=[[objects objectAtIndex:9] valueForKey:@"background"];
             
             PFFile *file1=[[objects objectAtIndex:9] valueForKey:@"afterMovie"];
...
- (UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if(indexPath.row == 0){
        ...
        if(!cell) {
            ...
            map.image=[UIImage imageNamed:@"maphide.jpg"];
            ...
            [_animatedLabel2 animateWithWords:@[
                                                @"Aftermovie",
                                                @"Hideout 2014 Official Highlights "
                                                ]                forDuration:5.0f
                                withAnimation:ATAnimationTypeFadeInOut];
        ...
        UIImageView *imageView = (UIImageView*)[cell viewWithTag:CONTENT_IMAGE_VIEW_TAG];
                imageView.image = [UIImage imageNamed:@"hid.content.png"];
    }
...
- (void)showInstagramFeed {
    ...
    instagram.instagramMultipleTags = @[@"hideoutfestival",
                                        @"hideout2014",@"hideout2015"];
    ...
    [self presentViewController: instagram.feed animated:YES completion:nil];
     [Flurry logEvent:@"Explore instragram-HIDEOUT"];
}
...
- (void)buttonPressed:(id)sender {
  ...
  [self dismissViewControllerAnimated:YES completion:nil];
           ...
           [Flurry logEvent:@"Back to main menu button- Hideout"];
}
...
-(void)lineup {
    ...
    JBWebViewController *controller = [[JBWebViewController alloc] initWithUrl:[NSURL URLWithString:@"http://www.hideoutfestival.com/2015-line-up"]];
    [controller showFromController:self];
         [Flurry logEvent:@"lineup- Hideout"];
}


-(void)ticket {
    JBWebViewController *controller = [[JBWebViewController alloc] initWithUrl:[NSURL URLWithString:@"http://www.hideoutfestival.com/about-hideout"]];
        [controller showFromController:self];
     [Flurry logEvent:@"ticket- Hideout"];
}



-(void)aftermovie {
    XCDYouTubeVideoPlayerViewController *videoPlayerViewController = [[XCDYouTubeVideoPlayerViewController alloc] initWithVideoIdentifier:@"NZJ7RHMpXBA"];
    [self presentMoviePlayerViewControllerAnimated:videoPlayerViewController];
        [Flurry logEvent:@"aftermovie- Hideout"];
}


-(void)facebook {
    JBWebViewController *controller = [[JBWebViewController alloc] initWithUrl:[NSURL URLWithString:@"https://www.facebook.com/hideoutfestival"]];
      [controller showFromController:self];
          [Flurry logEvent:@"facebook- Hideout"];
}



-(void)twitter {
    JBWebViewController *controller = [[JBWebViewController alloc] initWithUrl:[NSURL URLWithString:@"https://twitter.com/hideoutfestival"]];
       [controller showFromController:self];
...
- (void)map
{
    ...
    google.codinate = CLLocationCoordinate2DMake(44.540307,14.914657) ;
    google.codinate1 = CLLocationCoordinate2DMake(44.540307,14.914657) ;
