...
#import "coachellaViewController.h"
...
@interface coachellaViewController ()<MHYahooParallaxViewDatasource,MHYahooParallaxViewDelegate,UITableViewDelegate,UITableViewDataSource>
...
@implementation coachellaViewController
    ...
    [query findObjectsInBackgroundWithBlock:^(NSArray *objects, NSError *error)
     {
         if (!error)
         {
             PFFile *file=[[objects objectAtIndex:15] valueForKey:@"background"];
             
             PFFile *file1=[[objects objectAtIndex:15] valueForKey:@"afterMovie"];
...
- (UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if(indexPath.row == 0){
        ...
        if(!cell) {
            ...
            map.image=[UIImage imageNamed:@"mapcoach.jpg"];
        ...
        imageView.image = [UIImage imageNamed:@"coach-content.jpg"];
    }
...
- (void)showInstagramFeed {
    ...
    instagram.instagramMultipleUsersId = @[@"29551201", @"222040425",@"26853549"];
    
    instagram.instagramMultipleTags = @[@"coachella2015"];
    ...
    [self presentViewController: instagram.feed animated:YES completion:nil];
        [Flurry logEvent:@"Explore instragram-COACHELLA"];
}
...
- (void)buttonPressed:(id)sender {
    ...
    [Flurry logEvent:@"Back to main menu button- Tomorrowland"];
}
...
-(void)lineup {
    ...
    JBWebViewController *controller = [[JBWebViewController alloc] initWithUrl:[NSURL URLWithString:@"https://www.coachella.com/lineup/"]];
    
    [controller showFromController:self];
    [Flurry logEvent:@"lineup- coachella"];
}


-(void)ticket {
    JBWebViewController *controller = [[JBWebViewController alloc] initWithUrl:[NSURL URLWithString:@"https://www.coachella.com/festival-info/"]];
    [controller showFromController:self];
    [Flurry logEvent:@"ticket-coachella"];
}



-(void)aftermovie {
    XCDYouTubeVideoPlayerViewController *videoPlayerViewController = [[XCDYouTubeVideoPlayerViewController alloc] initWithVideoIdentifier:@"EoIbqjsTxJY"];
    [self presentMoviePlayerViewControllerAnimated:videoPlayerViewController];
    [Flurry logEvent:@"aftermovie-coachella"];
}


-(void)facebook {
    JBWebViewController *controller = [[JBWebViewController alloc] initWithUrl:[NSURL URLWithString:@"https://www.facebook.com/coachella"]];
    [controller showFromController:self];
    [Flurry logEvent:@"facebook- coachella"];    }



-(void)twitter {
    JBWebViewController *controller = [[JBWebViewController alloc] initWithUrl:[NSURL URLWithString:@"https://twitter.com/coachella"]];
    [controller showFromController:self];
    [Flurry logEvent:@"twitter- coachella"];
}
...
- (void)map
{
    ...
    google.codinate = CLLocationCoordinate2DMake(33.682185,-116.238283) ;
    google.codinate1 = CLLocationCoordinate2DMake(33.682185,-116.238283) ;
