...
@interface wirelessViewController ()<MHYahooParallaxViewDatasource,MHYahooParallaxViewDelegate,UITableViewDelegate,UITableViewDataSource>
    ...
    [query findObjectsInBackgroundWithBlock:^(NSArray *objects, NSError *error)
     {
         if (!error)
         {
             PFFile *file=[[objects objectAtIndex:8] valueForKey:@"background"];
             
             PFFile *file1=[[objects objectAtIndex:8] valueForKey:@"afterMovie"];
    ...
    if(indexPath.row == 0){
        ...
        if(!cell) {
            ...
            UIImageView *map =[[UIImageView alloc] initWithFrame:CGRectMake(0, 308, 320, 180)];
                map.image=[UIImage imageNamed:@"mapwire.jpg"];
        ...
        UIImageView *imageView = (UIImageView*)[cell viewWithTag:CONTENT_IMAGE_VIEW_TAG];
            imageView.image = [UIImage imageNamed:@"wire-content.png"];
    }
...
- (void)showInstagramFeed {
    ...
    instagram.instagramMultipleTags = @[@"wirelessfestival",@"wirelessfestival2014"];
    ...
    [self presentViewController: instagram.feed animated:YES completion:nil];
       [Flurry logEvent:@"Explore instragram-WIRELESS"];
}
...
- (void)buttonPressed:(id)sender {
  ...
  [self dismissViewControllerAnimated:YES completion:nil];
       ...
       [Flurry logEvent:@"Back to main menu button- WIRELESS"];
}
...
-(void)lineup {
    ...
    JBWebViewController *controller = [[JBWebViewController alloc] initWithUrl:[NSURL URLWithString:@"http://www.wirelessfestival.co.uk/lineup/artists"]];
    
 [controller showFromController:self];
           [Flurry logEvent:@"lineup- WIRELESS"];}


-(void)ticket {
    JBWebViewController *controller = [[JBWebViewController alloc] initWithUrl:[NSURL URLWithString:@"http://www.wirelessfestival.co.uk/info"]];
 [controller showFromController:self];
   [Flurry logEvent:@"ticket- WIRELESS"];}


-(void)aftermovie {
    XCDYouTubeVideoPlayerViewController *videoPlayerViewController = [[XCDYouTubeVideoPlayerViewController alloc] initWithVideoIdentifier:@"tALDj5jaGkU"];
    [self presentMoviePlayerViewControllerAnimated:videoPlayerViewController];
  [Flurry logEvent:@"aftermovie- WIRELESS"];}


-(void)facebook {
    JBWebViewController *controller = [[JBWebViewController alloc] initWithUrl:[NSURL URLWithString:@"https://www.facebook.com/WirelessFestival"]];
 [controller showFromController:self];
      [Flurry logEvent:@"facebook- WIRELESS"];}


-(void)twitter {
    JBWebViewController *controller = [[JBWebViewController alloc] initWithUrl:[NSURL URLWithString:@"https://www.twitter.com/wirelessfest"]];
 [controller showFromController:self];
       [Flurry logEvent:@"twitter- WIRELESS"];}
...
- (void)map
{
    ...
    google.codinate = CLLocationCoordinate2DMake(51.568712,-0.102706) ;
    google.codinate1 = CLLocationCoordinate2DMake(51.568712,-0.102706) ;
