...
@interface  isleViewController ()<MHYahooParallaxViewDatasource,MHYahooParallaxViewDelegate,UITableViewDelegate,UITableViewDataSource>
...
@implementation  isleViewController
    ...
    [query findObjectsInBackgroundWithBlock:^(NSArray *objects, NSError *error)
     {
         if (!error)
         {
             PFFile *file=[[objects objectAtIndex:2] valueForKey:@"background"];
             
             PFFile *file1=[[objects objectAtIndex:2] valueForKey:@"afterMovie"];
...
- (UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if(indexPath.row == 0){
        ...
        if(!cell) {
            ...
            map.image=[UIImage imageNamed:@"mapisle.jpg"];
     ...
     imageView.image = [UIImage imageNamed:@"isle-content.png"];
    }
...
- (void)showInstagramFeed {
    ...
    instagram.instagramMultipleTags = @[@"isleofwightfestival",@"isleofwightfest",
                                        @"isleofwightfestival2014"];
   ...
   [Flurry logEvent:@"Explore instragram-ISLE OF WIGHT"];
}
...
- (void)buttonPressed:(id)sender {
  ...
  [self dismissViewControllerAnimated:YES completion:nil];
        ...
        [Flurry logEvent:@"Back to main menu button- isle"];    
...
-(void)lineup {
 ...
 JBWebViewController *controller = [[JBWebViewController alloc] initWithUrl:[NSURL URLWithString:@"http://isleofwightfestival.com/line-up.aspx"]];
 [controller showFromController:self];
      [Flurry logEvent:@"lineup- isle"];
}


-(void)ticket {
    JBWebViewController *controller = [[JBWebViewController alloc] initWithUrl:[NSURL URLWithString:@"http://www.isleofwightfestival.com/info/"]];
 [controller showFromController:self];
            [Flurry logEvent:@"ticket- isle"];
}



-(void)aftermovie {
    XCDYouTubeVideoPlayerViewController *videoPlayerViewController = [[XCDYouTubeVideoPlayerViewController alloc] initWithVideoIdentifier:@"-vP1kazToyI"];
    [self presentMoviePlayerViewControllerAnimated:videoPlayerViewController];
          [Flurry logEvent:@"aftermovie- isle"];
}


-(void)facebook {
    JBWebViewController *controller = [[JBWebViewController alloc] initWithUrl:[NSURL URLWithString:@"https://www.facebook.com/iwfestival"]];
 [controller showFromController:self];
      [Flurry logEvent:@"facebook- isle"];
...
-(void)twitter {
    JBWebViewController *controller = [[JBWebViewController alloc] initWithUrl:[NSURL URLWithString:@"https://twitter.com/isleofwightfest"]];
 [controller showFromController:self];
           [Flurry logEvent:@"twitter- isle"];
}
...
- (void)map
{
    ...
    google.codinate = CLLocationCoordinate2DMake(50.708432, -1.28692) ;
     google.codinate1 = CLLocationCoordinate2DMake(50.708432, -1.28692) ;
